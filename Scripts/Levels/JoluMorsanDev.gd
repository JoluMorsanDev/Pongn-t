...
func _process(delta):
	if get_node_or_null("Jolumorsan12347/Yoshiazul") == null:
		win()
	else:
		$YoshiLifeLabel.text = "JLMSD LIFE: " + str(get_node_or_null("Jolumorsan12347/Yoshiazul").life)
...
func win():
	...
	$MainCam.global_rotation_degrees = 0
	$ColorRect.color = Color(0, 0, 0, 0)
	if Singletones.selectorlevelsunlocked == 7:
		Singletones.selectorlevelsunlocked = 8
	Singletones.levelsunlocked = 8
