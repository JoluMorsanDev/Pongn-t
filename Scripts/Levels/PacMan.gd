...
func _process(delta):
	if get_node_or_null("Pacboy").get_node_or_null("Pac") == null:
		win()
	else:
		$PacLifeLabel.text = "P. M. Life: " + str(get_node_or_null("Pacboy").get_node_or_null("Pac").life)
...
func win():
	...
	$MainCam.global_rotation_degrees = 0
	$ColorRect.color = Color(0, 0, 0, 0)
	if Singletones.selectorlevelsunlocked == 5:
		Singletones.selectorlevelsunlocked = 6
	Singletones.levelsunlocked = 6
