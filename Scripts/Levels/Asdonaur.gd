...
func _process(delta):
	if get_node_or_null("Asdo").get_node_or_null("Asdito") == null:
		win()
	else:
		$AsdoLifeLabel.text = "Asdo Life: " + str($Asdo/Asdito.life)
...
func win():
	...
	$MainCam.global_rotation_degrees = 0
	$ColorRect.color = Color(0, 0, 0, 0)
	if Singletones.selectorlevelsunlocked == 6:
		Singletones.selectorlevelsunlocked = 7
	Singletones.levelsunlocked = 7
